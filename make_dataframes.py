...
def add_lang(dataframe):
    ...
    dataframe['title_lang'] = dataframe['invention_title'].apply(detect_lang)
    dataframe['abstract_lang'] = dataframe['abstract'].apply(detect_lang)
    return dataframe

def extract_authors(publication_data):
    
    authors = []
    
    for data_entry in publication_data['data']:
...
def publication_table(json_data):
    ...
    df['source_subject'] = df['source'].apply(lambda x: x.get("asjc_subjects") if x else None)
    df['source_country'] = df['source'].apply(lambda x: x.get("country") if x else None )
